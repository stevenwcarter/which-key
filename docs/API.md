...
Every diagnostic which-key writes is prefixed `[whichkey]`. Nearly all of them are `console.warn` advisories — the library soft-fails on consumer misuse rather than throwing, so a warning means "this did not do what you meant", never "your app is about to crash". The exceptions are the two `console.error` rows at the end: they fire when _your own_ code throws — a shortcut handler or a `subscribe` listener — which is not consumer misuse of the which-key API, but which-key still catches it and keeps running rather than letting it propagate.
